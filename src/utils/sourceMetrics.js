...
export const SOURCE_FILTERS = [
  ...
  { id: 'email_sienna', label: 'Email - Sienna AI' },
...
export const SOURCE_ROOT_CAUSES = {
  ...
  email_human: 'Human email is concentrated in more complex refund, returns, and reimbursement work after routine volume is handled by Sienna. The lower FCR reflects policy judgment and multi-step resolution rather than Sienna routing drag.',
  email_sienna: 'Sienna is intentionally routed to routine email drivers and resolves most Order Status, Refund Requests, and Shipping & Delivery contacts quickly. Escalations are concentrated in Returns & Exchanges and Reimbursements & Adjustments where policy judgment is required.',
...
function topDrivers(records) {
  ...
  return [...counts.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([name, volume]) => ({ name, detail: `${volume.toLocaleString()} contacts` }))
}
...
export function buildSourceRowInsight(row) {
  return {
    ...
    contributorsLabel: 'Top 5 contact drivers by volume',
    contributorsCol1: 'Driver',
    contributorsCol2: 'Volume',
    contributors: row.topDrivers,
  }
}
